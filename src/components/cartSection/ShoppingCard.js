...
const ShoppingCard = (props) =>{
  ...
  <div className="flex flex-col bg-bgShoppingCard p-2 shadow-xl my-4">
    <h4 className="bg-Coffee p-2 font-bold">{props.item.descripcion}</h4>
    <div className ="flex flex-row">
      <div className="w-2/6 p-1">
        <img src = {props.item.imagen} w-full object-contain/>
      </div>
      <div className= "flex flex-col justify-between w-4/6 p-2">
        <p className="py-2">Subtotal: $ {props.item.precio}</p>
        <div>
            <IconButton icon = {<IconMore/>}/>
            <span className="text-xl">{props.item.cantidad}</span>
